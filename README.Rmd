...
**matrixStats** is`matrixStats::f(as.matrix(x))`. However, this of course 
realizes `x` in memory as a *base::matrix*, which typically defeats the entire 
purpose of using a *DelayedMatrix* for storing the data.
...
`colSums()`, `rowSums()`). This is a good start, but currently not all of the 
**matrixStats** API is currently supported. Furthermore, certain operations can 
be further operations with additional information about `x`. For example, if 
`x` is an *RleArray*, then `colSums(x)` can be very efficiently implemented by 
calling `sum,Rle-method()` on `x[, j]` for each `j`.
