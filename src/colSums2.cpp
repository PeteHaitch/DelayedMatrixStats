...
Rcpp::RObject colSums2_internal (X x, Rcpp::RObject na_rm) {

  // Checking matrix dimensions
  const size_t nrow = x->get_nrow();
  // TODO: Clarify use of process_subset_vector() [from scater]
...
SEXP colSums2 (SEXP x, SEXP na_rm) {
  BEGIN_RCPP

  auto x_type = beachmat::find_sexp_type(x);
  if (x_type == INTSXP) {
    auto x_ptr = beachmat::create_integer_matrix(x);
    return colSums2_internal<int, Rcpp::IntegerVector>(x_ptr.get(), na_rm);
  } else if (x_type == REALSXP) {
    auto x_ptr = beachmat::create_numeric_matrix(x);
    return colSums2_internal<double, Rcpp::NumericVector>(x_ptr.get(), na_rm);
  } else {
