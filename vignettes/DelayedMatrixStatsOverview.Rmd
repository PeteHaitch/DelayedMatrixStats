...
author: "Peter Hickey"
date: "Modified: 05 Oct 2017. Compiled: `r format(Sys.Date(), '%d %b %Y')`"
...
objects, e.g., packages wanting to retain backward compatability or during a 
...
set of column-wise and row-wise `r CRANpkg("matrixStats")` API[^api]. Please 
...
[a bug in `matrixStats::weightedVar()`](https://github.com/HenrikBengtsson/matrixStats/issues/105). `colAvgsPerRowSet()` will be implemented/reinstated upon the next CRAN release of **matrixStats** ([https://github.com/PeteHaitch/DelayedMatrixStats/issues/34](https://github.com/PeteHaitch/DelayedMatrixStats/issues/34))
...
DelayedMatrixStats <- sort(grep("^(col|row)", 
                         getNamespaceExports("DelayedMatrixStats"), 
                         value = TRUE))
...
```{r benchmarking, message = FALSE, echo = TRUE}
library(DelayedMatrixStats)
library(Matrix)
...
performance of these methods, specfically by developing additional 'seed-aware' 
